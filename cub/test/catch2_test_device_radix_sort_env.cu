...
TEST_CASE("Device radix sort keys descending DB decomposer works with default environment", "[radix_sort][device]")
{
  ...
  REQUIRE(
    cudaSuccess
    == cub::DeviceRadixSort::SortKeysDescending(d_keys, static_cast<int>(keys_buf0.size()), custom_decomposer_t{}));
...
TEST_CASE("Device radix sort pairs descending DB decomposer works with default environment", "[radix_sort][device]")
{
  ...
  REQUIRE(cudaSuccess
          == cub::DeviceRadixSort::SortPairsDescending(
            d_keys, d_values, static_cast<int>(keys_buf0.size()), custom_decomposer_t{}));
          ...
          "[radix_sort][device]")
{
  ...
  REQUIRE(
    cudaSuccess
    == cub::DeviceRadixSort::SortPairsDescending(
      d_keys, d_values, static_cast<int>(keys_buf0.size()), custom_decomposer_t{}, 0, static_cast<int>(sizeof(int) * 8)));
...
TEST_CASE("Device radix sort keys descending DB decomposer uses custom stream", "[radix_sort][device]")
{
  ...
  REQUIRE(cudaSuccess
          == cub::DeviceRadixSort::SortKeysDescending(
            d_keys, static_cast<int>(keys_buf0.size()), custom_decomposer_t{}, env));
